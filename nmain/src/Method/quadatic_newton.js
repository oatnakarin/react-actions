...
useEffect(() => {
    ...
    try {
      axios.get(`http://localhost:4000/quad/${token}`).then((respond)=>{
...
const quadratic_new = (arr1,arr2,point1,point2,point3,x) =>{
  ...
  if(point1 < 1 || point1 >arr1.length){
    ...
    alert("ระบุจุดที่ต้องการผิด!");
    return;
  }else if((point3 < 1 || point3 >arr1.length)){
    alert("ระบุจุดที่ต้องการผิด!");
    return;
  }
  let ans = [];
  ...
  let c2 = ((arr2[point3-1]-arr2[point2-1])/(arr1[point3-1]-arr1[point2-1])-(arr2[point2-1]-arr2[point1-1])/(arr1[point2-1]-arr1[point1-1]))/(arr1[point3-1]-arr1[point1-1]);
  let temp = "";
  ans.push(c0);ans.push(c1);ans.push(c2);
  for(let i=0;i<3;i++){
    if(i<2)
  ...
  setC(temp); temp = "";
  temp = "Answer: "+(c0+(c1*(x-arr1[point1-1]))+(c2*(x-arr1[point1-1])*(x-arr1[point2-1]))).toFixed(6);
  setans(temp)
    ...
    <div>
      <div style={{textAlign:'center',fontSize:'20px',fontWeight:'bold'}}>Quadratic Newton Divide Difference</div>
        <div style={{marginLeft:'150px'}}>
          ...
          <div style={{marginBottom:'15px',display:'flex',columnGap:'10px',alignItems:'center'}}>
            <label>Point 3</label>
            <input onChange={(e)=>setpoint3(e.target.value)} />
          </div>
          <div style={{display:'flex',gap:'20px'}}>
            <div>
              <Button type="primary" onClick={()=>quadratic_new(arr1,arr2,point1,point2,point3,x)}>Submit</Button>
            </div>   
          </div>    
        </div>  
        <div style={{margin:'50px 20px',columnGap:'20px',display:'flex',alignItems:'center'}}>
